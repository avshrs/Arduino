...
uint8_t readMcp(const uint8_t &MCP_ADDR,uint8_t &MPC_MEMORY, const  uint8_t MCP_RW_SIDE){  
    uint8_t value = 0; 
    Wire.begin();
    Wire.beginTransmission (MCP_RW_SIDE);  // expander has I2C address 0x20
    Wire.write (MCP_A_ADDR);   // register 0 is the I/O direction register for Port A
    Wire.endTransmission ();
